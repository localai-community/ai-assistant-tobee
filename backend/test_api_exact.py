...
async def test_api_exact():
    """Test exactly like the API."""
    
    print("🔍 Testing API Exact Behavior")
    print("=" * 50)
    ...
    try:
        ...
        print(f"\n🎯 Testing solve_problem (API style):")
        ...
        if result['confidence'] > 0:
            print(f"   ✅ SUCCESS: API exact test working")
        else:
            print(f"   ❌ FAILURE: API exact test not working")
            
            # Debug the exact same way
            print(f"\n🔍 Debugging API exact behavior:")
